...
class ImgUNet(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, num_levels=4, num_convs_per_level=2, base_channels=64, image_size=81, text_embedding_dim=1024):
        ...
        self.up_conv_1 = nn.Sequential(
            nn.Conv2d(512+text_embedding_dim, 512, kernel_size=3, padding=2, dtype=dtype, device=device),
            nn.ReLU(inplace=True),
            #LayerNorm(512+text_embedding_dim),
        )
        self.up_convs_1 = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(512, 512, kernel_size=1, padding=0, dtype=dtype, device=device),
                nn.ReLU(inplace=True),
                #LayerNorm(512),
                nn.Conv2d(512, 512, kernel_size=1, padding=0, dtype=dtype, device=device),
                nn.ReLU(inplace=True),
        ...
        self.up_conv_2 = nn.Sequential(
            nn.Conv2d(512, 256, kernel_size=3, padding=2, dtype=dtype, device=device),
            nn.ReLU(inplace=True),
            #LayerNorm(256),
        )
        self.up_convs_2 = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(256, 256, kernel_size=3, padding=2, dtype=dtype, device=device),
                nn.ReLU(inplace=True),
                #LayerNorm(256),
                nn.Conv2d(256, 256, kernel_size=3, padding=0, dtype=dtype, device=device),
                nn.ReLU(inplace=True),
                LayerNorm(256),
            ) for _ in range(3)
        ])
        self.up_conv_3 = nn.Sequential(
            nn.Conv2d(256, 128, kernel_size=3, padding=2, dtype=dtype, device=device),
            nn.ReLU(inplace=True),
            #LayerNorm(256),
        )
        self.up_convs_3 = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(128, 128, kernel_size=3, padding=2, dtype=dtype, device=device),
                nn.ReLU(inplace=True),
                #LayerNorm(128),
                nn.Conv2d(128, 128, kernel_size=3, padding=0, dtype=dtype, device=device),
                nn.ReLU(inplace=True),
                LayerNorm(128),
            ) for _ in range(3)
        ])
        self.up_conv_4 = nn.Sequential(
            nn.Conv2d(128, 64, kernel_size=3, padding=2, dtype=dtype, device=device),
            nn.ReLU(inplace=True),
            #LayerNorm(64),
        )
        self.up_convs_4 = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(64, 64, kernel_size=3, padding=2, dtype=dtype, device=device),
                nn.ReLU(inplace=True),
                #LayerNorm(64),
                nn.Conv2d(64, 64, kernel_size=3, padding=0, dtype=dtype, device=device),
                nn.ReLU(inplace=True),
                LayerNorm(64),
            ) for _ in range(3)
        ])
        self.out_conv = nn.Conv2d(64, out_channels, kernel_size=1, dtype=dtype, device=device)
    ...
    def forward(self, x, text_embedding):
        ...
        x = self.up_conv_1(x)
        for conv in self.up_convs_1:
            x = x + conv(x)
        x = self.up_conv_2(x)
        for conv in self.up_convs_2:
            x = x + conv(x)
        x = self.up_conv_3(x)
        for conv in self.up_convs_3:
            x = x + conv(x)
        x = self.up_conv_4(x)
        for conv in self.up_convs_4:
            x = x + conv(x)
        x = self.out_conv(x)
        x = F.sigmoid(x)
        ...
        for inputs, targets, text_embedding, text in data_loader:
            ...
            for i in range(len(inputs)):
                ...
                if i == 9 or i == 12:
                    ...
                    Image.fromarray((input_img * 255).astype(np.uint8)).save(f"{output_dir}/{i}_input.png")
                    Image.fromarray((output_img * 255).astype(np.uint8)).save(f"{output_dir}/{i}_output.png")
                    Image.fromarray((target_img * 255).astype(np.uint8)).save(f"{output_dir}/{i}_target.png")
                    # write text
